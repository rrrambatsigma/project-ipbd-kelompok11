...
DEPLOY_NAME="euro-news-preprocessing"
PREFECT_SERVER="http://prefect-server:4200/api"
MAX_RETRIES=30  # 30 × 10s = 5 menit max
PID_WORKER=""
...
shutdown() {
    echo ""
    echo "⏹️  SIGTERM diterima — menghentikan worker gracefully..."
...
# ═══════════════════════════════════════════
# WAIT FOR PREFECT SERVER (with timeout)
# ═══════════════════════════════════════════
...
asyncio.run(main())
" || echo "  ⚠️  Gagal create work pool (mungkin udah ada)"

# ═══════════════════════════════════════════
# IDEMPOTENT DEPLOYMENT — skip jika sudah ada
# ═══════════════════════════════════════════
...
async def main():
    async with get_client() as client:
        ...
        exists = any(d.name == 'euro-news-preprocessing' for d in deps)
        print(exists)
...
if [ "$DEP_EXISTS" = "True" ]; then
    ...
    prefect deploy --all 2>&1 || echo "  ⚠️  prefect deploy gagal"
fi
